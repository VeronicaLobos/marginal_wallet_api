...
UPDATED_PLANNED_EXPENSE_DATA = {"value": 450.0}


def test_create_planned_expense_success(auth_client: TestClient, test_auth_user: User):
    """
    ...
    * Cheks that the response data matches the input data.
    ...
    """
    response = auth_client.post("/planned_expenses/", json=PLANNED_EXPENSE_DATA)
...
def test_list_planned_expenses(auth_client: TestClient, test_auth_user: User):
    """
    ...
    * Checks that the response data contains the created planned
    ...
    """
    auth_client.post("/planned_expenses/", json=PLANNED_EXPENSE_DATA)
...
def test_get_planned_expense(auth_client: TestClient, test_auth_user: User):
    """
...
def test_update_planned_expense(auth_client: TestClient, test_auth_user: User):
    ...
    * Checks that the response data matches the updated data.
    ...
    response = auth_client.patch(
        "/planned_expenses/1", json=UPDATED_PLANNED_EXPENSE_DATA
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_delete_planned_expense(auth_client: TestClient, test_auth_user: User):
    """
